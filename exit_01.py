...
# --- UPDATED EXIT SETTINGS ---
st.sidebar.header("Exit Strategy Settings (RSI Cycle ONLY)")
st.sidebar.markdown("**The ONLY Exit:** Completion of the full Opposite RSI Cycle.")
st.sidebar.markdown("_No stop-loss or profit-taking limits are applied._")
# --- END UPDATED EXIT SETTINGS ---
...
# -----------------------
# Realignment detection and signal setting (ENTRY LOGIC - Unchanged)
# -----------------------
...
# New columns for Exit Logic
df['exit_signal'] = 0 # 1 cover short, -1 sell/exit long
df['exit_reason'] = None
...
# -----------------------
# 🚀 UPDATED: Trading State Management (Pure Opposite Cycle Exit Logic)
# -----------------------

def apply_trading_logic(df):
    """
    Simulates position tracking bar-by-bar, with the ONLY exit being the 
    completion of the full Opposite RSI Cycle. No other stops are used.
    """
    
    # State tracking variables
    position_status = 'FLAT'
    
    # Cycle tracking for the current trade
    in_cycle_after_entry = False # Has the RSI passed the first extreme (71/29) after entry?
    
    df_temp = df.copy() # Work on a copy to record exits
    
    # RSI Cycle thresholds
    RSI_OVERBOUGHT = 71
    RSI_OVERSOLD = 29
    
    for idx in df_temp.index:
        
        current_signal = df_temp.at[idx, 'signal'] # Entry signal from previous step
        current_rsi = df_temp.at[idx, 'RSI']
        
        exit_triggered = False

        # --- A. CHECK FOR EXIT CONDITIONS (If a position is currently open) ---
        if position_status == 'LONG':
            
            # --- 1. Primary Exit (Complete Opposite Cycle: Falling Cycle) ---
            
            # Part 1: Mark when the RSI hits the first extreme (71)
            # A Long position should naturally go to Overbought (71)
            if current_rsi >= RSI_OVERBOUGHT:
                in_cycle_after_entry = True
                
            # Part 2: Check for cycle completion (RSI must have hit 71, then fallen below 29)
            if in_cycle_after_entry and current_rsi <= RSI_OVERSOLD:
                df_temp.at[idx, 'exit_signal'] = -1 # Exit long
                df_temp.at[idx, 'exit_reason'] = "NATURAL EXIT: Complete Opposite (Falling) RSI Cycle"
                position_status = 'FLAT'
                in_cycle_after_entry = False
                exit_triggered = True


        elif position_status == 'SHORT':
                
            # --- 1. Primary Exit (Complete Opposite Cycle: Rising Cycle) ---
            
            # Part 1: Mark when the RSI hits the first extreme (29)
            # A Short position should naturally go to Oversold (29)
            if current_rsi <= RSI_OVERSOLD:
                in_cycle_after_entry = True
                
            # Part 2: Check for cycle completion (RSI must have hit 29, then risen above 71)
            if in_cycle_after_entry and current_rsi >= RSI_OVERBOUGHT:
                df_temp.at[idx, 'exit_signal'] = 1 # Cover short
                df_temp.at[idx, 'exit_reason'] = "NATURAL EXIT: Complete Opposite (Rising) RSI Cycle"
                position_status = 'FLAT'
                in_cycle_after_entry = False
                exit_triggered = True

        
        # --- B. CHECK FOR ENTRY CONDITIONS (Only if currently FLAT and no exit occurred) ---
        if position_status == 'FLAT' and not exit_triggered:
            if current_signal == 1:
                position_status = 'LONG'
                in_cycle_after_entry = False # Reset cycle tracking
...
# Apply the new trading logic to get the exit signals
df = apply_trading_logic(df)

# -----------------------
# Real-time Alerting (External + Internal) - Check BOTH Entry and Exit
# -----------------------
# We check the latest bar for *any* signal (entry or exit)
latest_activity = df[(df['signal'] != 0) | (df['exit_signal'] != 0)].tail(1)

if not latest_activity.empty:
    latest_time = latest_activity.index[0] 
    
    is_entry = latest_activity['signal'].iloc[0] != 0
    is_exit = latest_activity['exit_signal'].iloc[0] != 0
    
    if latest_time > st.session_state.last_signal_time:
        st.session_state.last_signal_time = latest_time
        
        if is_entry:
            signal_type = "BUY" if latest_activity['signal'].iloc[0] == 1 else "SELL"
            conviction_score = latest_activity['conviction_score'].iloc[0]
            lead_contributor = latest_activity['lead_contributor'].iloc[0]
            
            alert_message = (
                f"🔔 **NEW ENTRY ALERT ({signal_type})**: Cycle Realignment Fired!\n\n"
                f"**Time**: {latest_time.strftime('%Y-%m-%d %H:%M:%S')} ({interval})\n"
                f"**Action**: {signal_type} ({latest_activity['signal_reason'].iloc[0]})\n"
                f"**Conviction Score**: {conviction_score} / 4\n"
                f"**Lead Contributor**: **{lead_contributor}**"
            )
            ...
            action_type = "EXIT LONG" if latest_activity['exit_signal'].iloc[0] == -1 else "COVER SHORT"
            exit_reason = latest_activity['exit_reason'].iloc[0]
            
            alert_message = (
                f"🛑 **NEW EXIT ALERT ({action_type})**: Position Closed.\n\n"
                f"**Time**: {latest_time.strftime('%Y-%m-%d %H:%M:%S')} ({interval})\n"
                f"**Action**: {action_type}\n"
                f"**Reason**: **{exit_reason}**"
            )
            st.warning(alert_message, icon="⚠️")
            external_message = f"BTC/ETH Index EXIT ({interval}): {action_type} @ {latest_time.strftime('%H:%M')}. Reason: {exit_reason}."
            send_external_alert(action_type, external_message, recipient_email, recipient_phone, 0, 'N/A')
...
# 5. Exit Markers (Exit)
exit_long = df[df['exit_signal'] == -1]
cover_short = df[df['exit_signal'] == 1]
if not exit_long.empty:
    fig.add_trace(go.Scatter(x=exit_long.index, y=exit_long['index_cum_smooth'], mode='markers', marker_symbol='square',
                             marker_color='darkred', marker_size=10, name='EXIT LONG', marker_line_width=1), row=1, col=1)
if not cover_short.empty:
    fig.add_trace(go.Scatter(x=cover_short.index, y=cover_short['index_cum_smooth'], mode='markers', marker_symbol='square',
                             marker_color='darkgreen', marker_size=10, name='COVER SHORT', marker_line_width=1), row=1, col=1)
...
fig.add_hrect(y0=71, y1=100, fillcolor="red", opacity=0.1, line_width=0, row=2, col=1) # Original Overbought Zone
fig.add_hrect(y0=0, y1=29, fillcolor="green", opacity=0.1, line_width=0, row=2, col=1) # Original Oversold Zone

# The 50 line is removed as a trigger but kept for visual reference of the neutral zone
fig.add_hline(y=50, line=dict(color='gray', dash='dot', width=1), row=2, col=1, name='RSI 50 Neutral')
...
fig.update_layout(title="BTC/ETH Combined Index Momentum Dashboard (Entry & PURE Opposite RSI Cycle Exit)",
                  xaxis=dict(rangeslider=dict(visible=False)), height=1000, hovermode="x unified") # Increased height for third panel
st.plotly_chart(fig, use_container_width=True)

# -----------------------
# Show table of signals and diagnostics (Combined Entry and Exit)
# -----------------------
st.markdown("### Signals and Diagnostics (Entry & Exit)")
sig_df = df[(df['signal'] != 0) | (df['exit_signal'] != 0)].copy()

if sig_df.empty:
    st.info("No entry or exit signals found in the selected period with current parameters.")
else:
    # Combine signal and exit columns into a single action/reason
    sig_df['Action'] = np.where(sig_df['signal'] == 1, 'BUY (Entry)', 
                        np.where(sig_df['signal'] == -1, 'SELL (Entry)', 
                        np.where(sig_df['exit_signal'] == -1, 'EXIT LONG (Exit)',
                        np.where(sig_df['exit_signal'] == 1, 'COVER SHORT (Exit)', '---'))))
    
    sig_df['Reason/Conviction'] = np.where(sig_df['signal'] != 0, sig_df['signal_reason'], sig_df['exit_reason'])
    
    # Filter for final displayed columns
    display_cols = ['Index Price', 'RSI', 'EMA_short','EMA_long','EMA_very_long','ADX','PDI','NDI', 'Action', 'Reason/Conviction']
    
    sig_df['Index Price'] = sig_df['index_cum_smooth'].round(4)
    sig_df['RSI'] = sig_df['RSI'].round(2)
    sig_df['EMA_short'] = sig_df['EMA_short'].round(4)
    sig_df['EMA_long'] = sig_df['EMA_long'].round(4)
    sig_df['EMA_very_long'] = sig_df['EMA_very_long'].round(4)
    
    sig_df.index = sig_df.index.strftime('%Y-%m-%d %H:%M:%S')
    
    st.dataframe(sig_df[display_cols].tail(50))

# small metrics
st.markdown("### Summary")
st.write(f"Total cycles detected: **{len(cycles)}**")
st.write(f"Total ENTRY signals detected: **{int(df['signal'].abs().sum())}**")
st.write(f"Total EXIT signals generated: **{int(df['exit_signal'].abs().sum())}**")
st.write(f"Last activity timestamp recorded: **{st.session_state.last_signal_time.strftime('%Y-%m-%d %H:%M:%S')}**")
